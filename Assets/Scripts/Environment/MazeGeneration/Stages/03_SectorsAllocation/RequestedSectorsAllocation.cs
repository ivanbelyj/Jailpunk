...
public class RequestedSectorsAllocation : GenerationStage
{
    ...
    private Dictionary<int, List<SectorInfo>> requestedSectorsByGroupId;
    private Dictionary<int, SectorInfo> centersByGroupId;

    private HashSet<SectorInfo> allocatedRequestedSectors;
    ...
    private void AddSectorDebugColors(GenerationContext context) {
        ...
        Debug.Log("Distance between 5 and 7: " + GetDistance(context.SectorPossibleConnectivity, 5, 7));
    }
    #endregion
    ...
    private Dictionary<int, List<SectorInfo>> GetRequestedSectorsByGroupId(GenerationContext context)
    {
    ...
    private Dictionary<int, SectorInfo> GetCentersByGroupId() {
        return requestedSectorsByGroupId
            .ToDictionary(x => x.Key, x => GetGroupCenter(x.Value));
    }

    private SectorInfo GetGroupCenter(List<SectorInfo> groupSectors) {
        ...
        foreach (var (groupId, groupSectors) in requestedSectorsByGroupId) {
            foreach (var requestedSector in groupSectors) {
                ...
                if (!allocatedRequestedSectors.Contains(requestedSector)) {
                    AllocateNearCenter(context.SectorPossibleConnectivity, requestedSector);
                }
            }
        }
    }
