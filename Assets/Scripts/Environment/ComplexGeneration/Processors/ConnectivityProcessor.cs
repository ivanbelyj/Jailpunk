...
public class AreasBoundary
{
    ...
    // Todo: not very efficient solution
    public List<(Vector2Int, Vector2Int)> GetNeighboringBoundaryPairs()
    {
        var neighboringPairs = new List<(Vector2Int, Vector2Int)>();

        foreach (var tileA in BoundaryTilesA)
        {
            foreach (var tileB in BoundaryTilesB)
            {
                if (AreNeighbors(tileA, tileB))
                {
                    neighboringPairs.Add((tileA, tileB));
                }
            }
        }
    ...
    private bool AreNeighbors(Vector2Int tileA, Vector2Int tileB)
    {
        return Mathf.Abs(tileA.x - tileB.x) + Mathf.Abs(tileA.y - tileB.y) == 1;
    }
}
